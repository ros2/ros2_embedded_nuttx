...
static const int nofConfigTopics = /*7*/ 500;
static const int nofStateTopics = /*3*/ 100;
static const int nofStatisticTopics = /*3*/ 100;
...
unsigned int sleep(unsigned int seconds)
{
	if (seconds == 1)	/* Too long ... make it 1 ms :-) */
		...
		printf ("<linuxappl - memory/discovery status>\r\n");
		dbg_pool_dump ();
		dbg_disc_dump ();
...
void configure_minimal_pool_constraints (DDS_PoolConstraints *rp)
{
	...
	rp->min_local_readers = rp->max_local_readers = 508;
	rp->min_local_writers = rp->max_local_writers = 704;
	rp->min_topics = 1;
...
#define TopicDescriptionFromTopic(t)	(DDS_TopicDescription)(t)
...
//	unsigned iter = 0;
	...
	DDS_entity_name ("Technicolor Limits LinuxAppl");
...
//do {
	...
	for (i = 0; i < nofConfigTopics; i++) {
		configDataReader[i] = DDS_Subscriber_create_datareader(subscriber, 
			TopicDescriptionFromTopic(configTopic[i]), &dataReaderQos, NULL, 0);
...
//	sleep(TEST_TIME);
	...
	if (retCode != DDS_RETCODE_OK) {
        ...
        printf ("[%d] linuxappl completed successfully.\r\n", mstime() - startTime);
