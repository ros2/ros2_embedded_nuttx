...
#define USER_LAST (USER_FIRST + USER_NUM - 1)

static int _num = USER_NUM;
static int _rd_policy_updates = 0;
static int _wr_policy_updates = 0;

static int64_t _left = 0;
static int64_t _self = 0;
static int64_t _right = 0;

static bool _dir = true; /* forward */
                                  ...
                                  bool reverse /* use reverse dir */)
{
    ...
    bool d = (reverse ? !_dir : _dir);

    if (d /* forward */) {
        perm = (other == _right ? QEO_POLICY_ALLOW : QEO_POLICY_DENY);
    }
    else /* backward */ {
        perm = (other == _left ? QEO_POLICY_ALLOW : QEO_POLICY_DENY);
    }
    log_self("policy update for uid %" PRIx64 " -> going %s : %s",
            other, (d ? "forward" : "backward"),
            (QEO_POLICY_ALLOW == perm ? "allow" : "deny"));
    return perm;
}
                    ...
                    uintptr_t userdata)
{
    ...
    if (_dir /* forward*/) {
        log_self("got %" PRIx64 " expected %" PRIx64, tt->key, _right);
        assert(_right == tt->key);
    }
    else {
        log_self("got %" PRIx64 " expected %" PRIx64, tt->key, _left);
        assert(_left == tt->key);
    }
    sem_post(&_data_sync);
}
...
static void set_self(int64_t self)
{
    ...
    _left = (USER_FIRST == _self ? USER_LAST : (_self - 1));
    _right = (USER_LAST == _self ? USER_FIRST : (_self + 1));
}
...
static void run_test()
{
    ...
    /* change direction */
    _dir = !_dir;
    /* trigger policy update (first writer!) */
    sleep(2);
    assert(QEO_OK == qeo_state_writer_policy_update(writer));
    assert(QEO_OK == qeo_state_change_reader_policy_update(reader));
    /* wait for new policy update (reader and writer) */
    sem_wait(&_pol_sync);
    sem_wait(&_pol_sync);
    log_self("policies updated again");
    /* wait for data reception */
    sem_wait(&_data_sync);
    log_self("backward data received");
