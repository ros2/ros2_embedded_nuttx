...
static void my_on_data_available(const qeocore_reader_t *reader,
                                 ...
                                 uintptr_t userdata)
{
    // the reader is a type4 reader
    switch (qeocore_data_get_status(data)) {
        case QEOCORE_DATA: {

            int int32_value_rx=0, int8_value_rx=0;
            int int32_2_value_rx=0, int8_2_value_rx=0 ;
            char* string_value_rx="";
            char* string_2_value_rx="";
            log_pid("reader received data");
            assert(QEO_OK == qeocore_data_get_member(data, _int32_2_id, &int32_2_value_rx));
            assert(QEO_OK == qeocore_data_get_member(data, _int8_2_id, &int8_2_value_rx));
            assert(QEO_OK == qeocore_data_get_member(data, _string_2_id, &string_2_value_rx));
            ...
            log_verbose(" =================== int32_2_value_rx = %u \n", int32_2_value_rx );
            log_verbose(" =================== int8_2_value_rx = %u \n", int8_2_value_rx );
            log_verbose(" =================== string_2_value_rx = \"%s\" \n", string_2_value_rx );
...
static qeocore_type_t *type4_register(qeo_factory_t *factory)
{
    qeocore_type_t *dyndata = NULL;
    qeocore_type_t *primitive = NULL;
...
static void run_reader(void)
{
    ...
    assert(NULL != (factory = qeocore_factory_new(QEO_IDENTITY_DEFAULT)));
    init_factory(factory);
    assert(NULL != (type4 = type4_register(factory)));
    assert(NULL != (reader = qeocore_reader_open(factory, type4, NULL,
                                                 QEOCORE_EFLAG_STATE_DATA | QEOCORE_EFLAG_ENABLE,
                                                 &listener, NULL)));
