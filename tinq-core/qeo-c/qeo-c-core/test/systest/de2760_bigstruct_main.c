...
static qeo_retcode_t write_generated_data(qeocore_writer_t *writer, int arraysize, int stringsize){
    org_qeo_dynamic_qdm_test_TypeWithStructs_t t = {};
    size_t i, j;
    qeo_retcode_t rc = QEO_EFAIL;
    char mainstring[stringsize];

    t.mfloat32 = 32;
    t.mstring = str_random(mainstring, stringsize);
...
int main(int argc, const char **argv)
{
    ...
    /* send structure */
    assert(QEO_OK == write_generated_data(writer, 100, 10));
    assert(QEO_OK == write_generated_data(writer, 0, 100000));
