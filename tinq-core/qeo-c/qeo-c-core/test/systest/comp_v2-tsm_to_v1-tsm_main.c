                              ...
                              uintptr_t userdata)
{
    ...
    switch (qeocore_data_get_status(data)) {
        case QEOCORE_DATA: {
            ...
            assert(0 == strcmp(_type_qdm_version2.string, t->string));
            assert(_type_qdm_version2.i8 == t->i8);
            assert(_type_qdm_version2.i16 == t->i16);
            assert(_type_qdm_version2.i32 == t->i32);
            assert(_type_qdm_version2.i64 == t->i64);
            assert(_type_qdm_version2.f32 == t->f32);
            assert(_type_qdm_version2.boolean == t->boolean);
            /* release main thread */
...
int main(int argc, const char **argv)
{
    ...
    if (pid == -1) {
       ...
       log_verbose(" =================== c qeocore_type_register_tsm(factory, _tsm_type_qdm_version2, _tsm_type_qdm_version2[0].name) \n");
       assert(NULL != (type2 = qeocore_type_register_tsm(factory, _tsm_type_qdm_version2, _tsm_type_qdm_version2[0].name)));
       log_verbose(" =================== c qeocore_writer_open(factory, type2, NULL, QEOCORE_EFLAG_EVENT_DATA, NULL, NULL) \n");
       assert(NULL != (writer = qeocore_writer_open(factory, type2, NULL, QEOCORE_EFLAG_EVENT_DATA, NULL, NULL)));
       ...
       /* send structure */
       sleep(2);
       log_verbose(" =================== c qeocore_writer_write(writer, &_type_qdm_version2) \n");
       assert(QEO_OK == qeocore_writer_write(writer, &_type_qdm_version2));
       sleep(2);
       /* clean up */
