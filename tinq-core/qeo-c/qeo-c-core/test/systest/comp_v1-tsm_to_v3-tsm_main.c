...
static void on_data_available(const qeocore_reader_t *reader,
                              ...
                              uintptr_t userdata)
{
    log_verbose(" =================== %d on_data_available( is a type3 reader) \n", getpid());
    switch (qeocore_data_get_status(data)) {
        case QEOCORE_DATA: {
            log_verbose(" =================== case QEOCORE_DATA \n");
            type_qdm_version3_t *t = (type_qdm_version3_t *)qeocore_data_get_data(data);
            /* verify struct */
...
int main(int argc, const char **argv)
{
    ...
    if (pid == -1) {
       ...
       assert(NULL != (factory = qeocore_factory_new(QEO_IDENTITY_DEFAULT)));
       init_factory(factory);
       log_verbose(" =================== p qeocore_type_register_tsm(factory, _tsm_type_qdm_version3, _tsm_type_qdm_version3[0].name) \n");
       assert(NULL != (type3 = qeocore_type_register_tsm(factory, _tsm_type_qdm_version3, _tsm_type_qdm_version3[0].name)));
       log_verbose(" =================== p qeocore_reader_open(factory, type3, NULL, QEOCORE_EFLAG_EVENT_DATA, &listener, NULL)  \n");
       assert(NULL != (reader = qeocore_reader_open(factory, type3, NULL, QEOCORE_EFLAG_EVENT_DATA, &listener, NULL)));
       /* test late enabling of readers/writers */
