				...
				DDS_InstanceHandle_t   h)
{
	...
	do {
		rc = DDS_DynamicData_set_int32_value (d, X_ID, data->x_);
		if (rc)
			break;

		rc = DDS_DynamicData_set_int32_value (d, Y_ID, data->y_);
		if (rc)
			break;

		rc = DDS_DynamicData_set_int32_value (d, Z_ID, data->z_);
		if (rc)
			break;
...
static DDS_ReturnCode_t get_double (DDS_DynamicData d, double *s, DDS_MemberId id)
{
	...
	rc = DDS_DynamicData_get_int32_value (d, s, id);
	if (rc)
		return (rc);
