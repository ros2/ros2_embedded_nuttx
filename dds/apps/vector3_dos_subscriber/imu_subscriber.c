...
static void *dds_send_imu (void *args)
{
	...
	for (;;){
		...
		/* According to https://github.com/brunodebus/tinq-core/issues/7#issuecomment-63740498:
			the Vector3_dos shouldn't be registered if it doesn't contain a @key attribute
		*/		
		if (!h)
			h = Vector3_dos_register (dw, &m);
#endif
...
int main (int argc, const char **argv)
{
	...
	ts = Vector3_dos_type_new ();
	if (!ts) {
		printf ("Can't create vector3 message type!\r\n");
		exit (1);
	}
	error = DDS_DynamicTypeSupport_register_type (ts, part, "simple_msgs::dds_::Vector3_dos_");
	if (error) {
		printf ("Can't register vector3 message type.\r\n");
		exit (1);
	}
	if (verbose)
		printf ("DDS Topic type ('%s') registered.\r\n", "simple_msgs::dds_::Vector3_dos_");

	topic = DDS_DomainParticipant_create_topic (part, "imu", "simple_msgs::dds_::Vector3_dos_", NULL, NULL, 0);
	if (!topic) {
		printf ("Can't register vector3 message type.\r\n");
		exit (1);
	}
	if (verbose)
		printf ("DDS Vector3_dos Topic created.\r\n");
	...
	if (verbose)
		printf ("DDS Vector3_dos message writer created.\r\n");
	...
	Vector3_dos_type_free (ts);
	if (verbose)
		printf ("Vector3_dos Type deleted.\r\n");
